...
function renderHistory() {
  ...
  if (expBtn) expBtn.addEventListener("click", function () {
    ...
    window.TarotPrompt.playSfx(PRESS_SFX);
    // Android WebView 无文件下载界面：导出改为复制到剪贴板，Toast 文案同步说明
    window.TarotPrompt.showToast(ok ? (window.AndroidBridge ? "已导出全部历史记录（已复制到剪贴板）" : "已导出全部历史记录") : "暂无记录可导出");
  });
...
function copyText(text) {
  // Android WebView 环境：优先调用原生剪贴板桥（WebView 中 navigator.clipboard/execCommand 不可靠）
  if (window.AndroidBridge && typeof window.AndroidBridge.copyText === "function") {
    ...
    window.TarotPrompt.showToast(copied ? "提示词已复制" : "复制失败，请手动长按选择复制");
    return;
  }
  var ok = false;
  function fallback() {
